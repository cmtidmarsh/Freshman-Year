...
def main():
    ...
    while True:
        ...
        e = rn.randint(0,500)
        rnpos = rn.choice([leftLines,rightLines])
        rnline = rn.choice(rnpos)
        e1 = rnline.y - 10

        rneffect = generateEffects(e,e1)

        if sp.rectangle.bottom <= e1:
            num = rn.randint(1,11)

            if num == 10 and generated == False:
                    rneffect.draw(screen)
                    generated = True
